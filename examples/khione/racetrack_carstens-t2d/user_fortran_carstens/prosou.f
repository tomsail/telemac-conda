...
!
!+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
!     PROPELLER (USER_DEFINED)
      DOUBLE PRECISION                 :: XSOM(4),YSOM(4)
      DOUBLE PRECISION                 :: FPROP, UPROP, VPROP
!+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
!
...
!
  ...
  !
        DO N=1,NPOIN
          IF(INPOLY(MESH%X%R(N),MESH%Y%R(N),XSOM,YSOM,4)) THEN
            UPROP = -0.52D0 - U%R(N)
            VPROP =  0.0D0  - V%R(N)
            WD = SQRT(UPROP**2 + VPROP**2)
            FU%R(N) = FU%R(N) + FPROP * UPROP * WD / HN%R(N)
            FV%R(N) = FV%R(N) + FPROP * VPROP * WD / HN%R(N)
          ENDIF
        ENDDO
!
!*****************************************************************************
