...
class VnvStudy(AbstractVnvStudy):
    ...
    def _pre(self):
        ...
        """
        casname='t2d_diffusion_FV_advection.cas'
        ...
        cas.set('DIFFUSION OF TRACERS', False)
        self.add_study('nodiff_seq', 'telemac2d', casname, cas=cas)
        cas.set('PARALLEL PROCESSORS', 4)
        self.add_study('nodiff_par', 'telemac2d', casname, cas=cas)
        del cas
    ...
    def _check_results(self):
        ...
        """
        if self.mesh_type==3:
            self.check_epsilons('tpf_seq:T2DRES', 'f2d_diffusion_adv_tpf.slf', eps=[1e-6])
            self.check_epsilons('tpf_par:T2DRES', 'f2d_diffusion_adv_tpf.slf', eps=[1e-6])
            self.check_epsilons('tpf_seq:T2DRES', 'tpf_par:T2DRES', eps=[1e-6])

            self.check_epsilons('hefe_seq:T2DRES', 'f2d_diffusion_adv_hefe.slf', eps=[1e-6])
            self.check_epsilons('hefe_par:T2DRES', 'f2d_diffusion_adv_hefe.slf', eps=[1e-6])
            self.check_epsilons('hefe_seq:T2DRES', 'hefe_par:T2DRES', eps=[1e-6])

            self.check_epsilons('rtpf_seq:T2DRES', 'f2d_diffusion_adv_rtpf.slf', eps=[1e-6])
            self.check_epsilons('rtpf_par:T2DRES', 'f2d_diffusion_adv_rtpf.slf', eps=[1e-6])
            self.check_epsilons('rtpf_seq:T2DRES', 'rtpf_par:T2DRES', eps=[1e-6])

            self.check_epsilons('nodiff_seq:T2DRES', 'f2d_diffusion_adv_nodiff.slf', eps=[1e-6])
            self.check_epsilons('nodiff_par:T2DRES', 'f2d_diffusion_adv_nodiff.slf', eps=[1e-6])
            self.check_epsilons('nodiff_seq:T2DRES', 'nodiff_par:T2DRES', eps=[1e-6])
    ...
    def _post(self):
        ...
        for idx, record in enumerate(records):
            vnv_plot1d_polylines(\
                ...
                x_label='x (m)',
                ylim=[0,1],
                fig_name='img/t2d_diffusiontracer_adv_vf_T1_{}'.format(record),
                plot_bottom=False,
                markers=True,
                markevery=15)
            ...
            vnv_plot1d_polylines(\
                ...
                x_label='x (m)',
                ylim=[0,1],
                fig_name='img/t2d_diffusiontracer_adv_vf_T2_{}'.format(record),
                plot_bottom=False,
                markers=True,
                markevery=15)
