...
class VnvStudy(AbstractVnvStudy):
    ...
    def _pre(self):
        ...
        # pluie parallel mode
        cas = TelemacCas('t2d_pluie_ga.cas', get_dico('telemac2d'))
        cas.set('PARALLEL PROCESSORS', 4)

        self.add_study('vnv_2',
                       'telemac2d',
                       't2d_pluie_par_ga.cas',
                       cas=cas)
        del cas
        ...
        self.add_study('vnv_5',
                       'telemac2d',
                       't2d_pluie_ks_geo_hyetograph_ga.cas')


        # rain with hyetopgraph model parallel mode
        cas = TelemacCas('t2d_pluie_ks_geo_hyetograph_ga.cas', get_dico('telemac2d'))
        cas.set('PARALLEL PROCESSORS', 4)

        self.add_study('vnv_6',
                       'telemac2d',
                       't2d_pluie_ks_geo_hyetograph_par_ga.cas',
                       cas=cas)
        del cas
    ...
    def _post(self):
        ...
        vnv_plot2d(\
                '',res,
                plot_mesh=True,
                fig_name='img/mesh_ga',
                fig_size=(5,4),
                x_label='x (m)',y_label='y (m)')

        #Plot the ks coefficients
        vnv_plot2d(\
            'KS COEFFICIENT',
            res2,
            record=-1,
