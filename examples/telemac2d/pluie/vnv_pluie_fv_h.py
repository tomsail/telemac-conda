...
class VnvStudy(AbstractVnvStudy):
    ...
    def _pre(self):
        ...
        # pluie parallel mode
        cas = TelemacCas('t2d_pluie_fv_h.cas', get_dico('telemac2d'))
        cas.set('PARALLEL PROCESSORS', 4)

        self.add_study('vnv_2',
                       'telemac2d',
                       't2d_pluie_fv_par_h.cas',
                       cas=cas)
        del cas
        ...
        self.add_study('vnv_3',
                       'telemac2d',
                       't2d_pluie_fc_fv_h.cas')


        # rain with runoff model parallel mode
        cas = TelemacCas('t2d_pluie_fc_fv_h.cas', get_dico('telemac2d'))
        cas.set('PARALLEL PROCESSORS', 4)

        self.add_study('vnv_4',
                       'telemac2d',
                       't2d_pluie_fc_fv_par_h.cas',
                       cas=cas)
        del cas
        ...
        self.add_study('vnv_5',
                       'telemac2d',
                       't2d_pluie_fc_geo_hyetograph_fv_h.cas')


        # rain with hyetopgraph model parallel mode
        cas = TelemacCas('t2d_pluie_fc_geo_hyetograph_fv_h.cas', get_dico('telemac2d'))
        cas.set('PARALLEL PROCESSORS', 4)

        self.add_study('vnv_6',
                       'telemac2d',
                       't2d_pluie_fc_geo_hyetograph_fv_par_h.cas',
                       cas=cas)
        del cas
    ...
    def _post(self):
        ...
        vnv_plot2d(\
            ...
            record=-1,
            fig_size=(5,4),
            fig_name='img/water-depth1-fv_h',
