      ...
      IF(LT.EQ.NIT) THEN
        ...
        DO I = 1, NPERIAF
          IF(NPTS.GT.0) THEN
            ...
            WRITE(LU,*) 'MY ID',ID
            IF(IPID.EQ.0) THEN
              WRITE(ID,*) 'ANALYSE OF PERIOD ', PERIAF(I), ' S :'
            ENDIF
            WRITE(LU,*) ' '
            WRITE(LU,90) 'NOM DE POINT', 'AMPLITUDE', 'PHASE'

            IF(IPID.EQ.0) THEN
              WRITE(ID,90) 'NOM DE POINT', 'AMPLITUDE', 'PHASE'
            ENDIF
            WRITE(LU,*) ' '
            DO J = 1, NPTS
