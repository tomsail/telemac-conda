...
def main(recompile=True):
    ...
    cas_file = path.join(root, 'examples', 'artemis', 'bj78',
                         'art_bj78.cas')
    ...
    vnv_working_dir = vnv_copy_files('artemis', cas_file, par)

    chdir(vnv_working_dir)

    # Creation of the instance Telemac3d
    study = Artemis(path.basename(cas_file), user_fortran='f90', comm=comm, stdout=0,
                    recompile=recompile)
