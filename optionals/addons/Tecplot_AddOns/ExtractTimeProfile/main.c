  ...
  *ErrMsg = TecUtilStringAlloc(1000,"String for Error Message");
	      ...
	      &ResIsTransient,&NbPoints,&X[0],&Y[0],&Z[0],&X[1],&Y[1],&Z[1]))
  {
        sprintf(str," Nb Points : %d \n %d \n %lf %lf %lf \n %lf %lf %lf \n",
	      NbPoints,ResIsTransient,X[0],Y[0],Z[0],X[1],Y[1],Z[1]);
        TRACE(str);
       TRACE("Avant CheckIfExtractionIsOk \n");
/*       IsOk = CheckIfExtractionIsOk(); 
       sprintf(str,"ok? %d \n",IsOk);
       TRACE(str);
*/
       sprintf(str,"NbPoints %d \n",NbPoints);
       TRACE(str);
       sprintf(str,"&NbPoints %d \n",&NbPoints);
       TRACE(str);
       IsOk = 1;
       if(IsOk){
         TRACE("Avant extraction");
         IsOk = extraction(X,Y,Z,NbPoints,ResIsTransient);
         //IsOk = extraction(X,Y,Z,10,1);
         sprintf(str,"ok? %d \n",IsOk);
         TRACE(str);
        }  
  }
