...
/* Local Variables */
  ...
  LgIndex_t Debug = 1;      /* Debug for tecplot writing */
  LgIndex_t IsDouble = 1 ; // data are in double precision
...
/************************************************************************/
  ...
  TRACE("after reading variable names\n");
  for(i=0;i<NbVar+NbVarAux-1;i++)
  {
     VarNames[(i+1)*30-1]=',';
  }
  TRACE("after transforming variable names\n");
  ...
  while (!EndOfFile) 
  {
    ...
    if(TecOk == FALSE )
    {
        TRACE("Error creating SV_VARSHAREZONELIST argument.\n");
	return(FALSE);
    }
    ...
    if(TecOk == FALSE)
    {
        TRACE("Error creating SV_PASSIVEVARLIST argument.\n");
	return(FALSE);
    }
    ...
    if(TecOk == FALSE  )
    {
        TRACE("Error creating SV_ISBLOCK argument.\n");
	return(FALSE);
    }
    ...
    if(TecOk == FALSE)
    {
        TRACE("Error creating STRANDID argument.\n");
	return(FALSE);
    }
    ...
    if(TecOk == FALSE )
    {
        TRACE("Error creating tecplot zone.\n");
	return(FALSE);
    }
