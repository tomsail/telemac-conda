 ...
 *----------------------------------------------------------------------*/
  ...
  if(fread(x,npoin*4,1,f)!=1)
  {
      strcpy(*MessageString,"Error reading x coordinates");
  }
  if(Invert) { for(i=0;i<npoin;i++) { swapbytes32(x[i]); } }
  ...
  if(fread(y,npoin*4,1,f)!=1)
  {
      strcpy(*MessageString,"Error reading x coordinates");
  }

  if(Invert) { for(i=0;i<npoin;i++) { swapbytes32(y[i]); } }
  ...
  /*
  strcpy(*MessageString ,"exit ReadMesh - status = ok");
  TecUtilDialogMessageBox(*MessageString,MessageBox_Information);
  */
