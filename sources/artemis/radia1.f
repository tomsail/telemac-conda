...
!
      IF(IRADIA.EQ.1) THEN
!
      CALL OS('X=YZ    ' , X=T3, Y=PHII, Z=PHII)
      CALL OS('X=X+YZ  ' , X=T3, Y=PHIR, Z=PHIR)
...
!
     ...
     &            MESH , MSK , MASKEL)
!
      CALL OS('X=Y+Z   ', X=T4, Y=T2 , Z=T4)
      CALL OS('X=Y/Z   ', X=T4, Y=T4 , Z=T1)
      CALL OS('X=Y/Z   ', X=T4, Y=T4 , Z=T1)
!
      CALL OS('X=CY/Z  ', X=T2, Y=CG , Z=C, C=2.D0  )
      CALL OS('X=X+C   ', X=T2, C=-1.D0 )
      OMEG2 = OMEGA*OMEGA
      CALL OS('X=CX    ' , X=T2 , C=OMEG2 )
!
      COE = 1.D0/(8.D0*GRAV)
!
!=======================================================================
