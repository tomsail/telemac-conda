...
subroutine INIT_ETAT_TRACER(Erreur, Identifiant, Impression)
   ...
   integer :: k
   integer :: nb_sect, nb_bief
   character(LEN=256)        :: MessageErreur

   Erreur = TEST_INIT_AND_ID(Identifiant, 'INIT_ETAT_TRACER')
   if (Erreur > 0 ) then
      return
   end if
   ...
   if( Modele%Tracer%Presence_ConcIni ) then
      call LEC_CONC_INI_TRACER_INTERFACE(       &
                                          ult , &
              Modele%Tracer%ImpressionConcIni , &
                Modele%Tracer%FichierConcInit , &
                                     Modele%X , & ! Maillage
                          Modele%Typemaillage , & ! Mode de calcul du maillage
            Masc%EtatMascaret%Tracer%Ctraceur , & ! Concentrations initiales des traceurs
                         Modele%Tracer%Nbtrac , & ! nombre de traceurs
                               Modele%Connect , & ! Table de connectivite du reseau
