...
!*************************************************************************
   ...
   if( impression ) write(UniteListing,10000)
   ...
   do iloi = 1 , nb_loi_tracer
     ...
     read(unit=line, fmt=*) mode_entree_loi
      ...
      if( mode_entree_loi == SAISIE_PAR_FICHIER ) then
         ...
         if( impression ) then
            write(UniteListing,10030) 'PAR FICHIER' , FichiersLoisTracer(iFichiersloi)%Nom
         endif

         call LEC_FIC_LOI_TRACER( &
            FichiersLoisTracer(iFichiersloi) , & ! Fic. contenant une evolution temporelle de conc
