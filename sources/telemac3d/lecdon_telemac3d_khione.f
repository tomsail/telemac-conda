!                   **********************************
                    SUBROUTINE LECDON_TELEMAC3D_KHIONE
!                   **********************************
!
...
!
      IF (NC_FRA.GT.0) THEN
        ...
        ALLOCATE(SLVFRZL(1))
        ALLOCATE(DNUFRZLH(NC_FRA))
        ALLOCATE(DNUFRZLV(NC_FRA))
...
!       ASSUMING A SINGLE VALUE FOR ALL SOLVERS
        ...
        DO I=1,NC_FRA
!         INITIALIZATION OF COEFFICIENTS FOR DIFFUSION
          DNUFRZLH(I)=1.D-6
          DNUFRZLV(I)=1.D-6
...
!         IN THE RIGHT ORDER BND1(T1;T2;...); BND2(T1....
        ...
        ENDIF
!       DIFFUSION SCHEME
        SCHDFRZL = MOTINT(ADRESS(1,7))
!       HORIZONTAL AND VERTICAL DIFFUSION COEFFICIENTS
        IF(TROUVE(2,10).EQ.2.AND.DIMENS(2,10).EQ.NC_FRA) THEN
          DO I=1,NC_FRA
            DNUFRZLH(I) = MOTREA(ADRESS(2,10)+I-1)
          ENDDO
        ELSEIF(TROUVE(2,10).EQ.2.AND.DIMENS(2,10).LT.NC_FRA
     &    .AND.DIMENS(2,10).GT.0) THEN
!         READING WHAT HAS BEEN GIVEN
          DO I=1,DIMENS(2,10)
            DNUFRZLH(I) = MOTREA(ADRESS(2,10)+I-1)
          ENDDO
!         COMPLETING WITH THE LAST GIVEN
          DO I=DIMENS(2,10)+1,NC_FRA
            DNUFRZLH(I) = MOTREA(ADRESS(2,10)+DIMENS(2,10)-1)
          ENDDO
        ENDIF
        IF(TROUVE(2,15).EQ.2.AND.DIMENS(2,15).EQ.NC_FRA) THEN
          DO I=1,NC_FRA
            DNUFRZLV(I) = MOTREA(ADRESS(2,15)+I-1)
          ENDDO
        ELSEIF(TROUVE(2,15).EQ.2.AND.DIMENS(2,15).LT.NC_FRA
     &    .AND.DIMENS(2,15).GT.0) THEN
!         READING WHAT HAS BEEN GIVEN
          DO I=1,DIMENS(2,15)
            DNUFRZLV(I) = MOTREA(ADRESS(2,15)+I-1)
          ENDDO
!         COMPLETING WITH THE LAST GIVEN
          DO I=DIMENS(2,15)+1,NC_FRA
            DNUFRZLV(I) = MOTREA(ADRESS(2,15)+DIMENS(2,15)-1)
          ENDDO
        ENDIF
!       WARNING:COEFFICIENT FOR DIFFUSION OF SUSPENDED SEDIMENTS
!       IS ONLY FOR 2D SIMULATIONS
        IF(TROUVE(2,9).EQ.2) THEN
          WRITE(LU,*)'WARNING: COEFFICIENT FOR DIFFUSION OF'
          WRITE(LU,*)'SUSPENDED SEDIMENTS IS NOT USED IN 3D:'
          WRITE(LU,*)'YOU NEED TO SET THE HORIZONTAL AND VERTICAL ONE'
        ENDIF
      ENDIF
!
