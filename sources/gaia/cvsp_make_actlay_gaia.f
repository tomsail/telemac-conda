...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ...
      USE INTERFACE_GAIA_BEDLOAD, EX => CVSP_MAKE_ACTLAY_GAIA
...
!
      INTEGER  I,J,K,JG, II
      DOUBLE PRECISION TEMP, Z_HIGH, Z_LOW
...
!-----------------------------------------------------------------------
      CALL  CVSP_CHECK_MASS_BILAN_GAIA()
!-----------------------------------------------------------------------
! NODES LOOP
!-----------------------------------------------------------------------
      DO J=1,NPOIN
...
!-----------------------------------------------------------------------
        ...
        NLAYER%I(J) = NOMBLAY
        NEW_ALT =  MIN( ZSPACE, CVSP_ALT_GAIA(J,ALT_MODEL))
!
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
        ...
        DO K=1,NOMBLAY
          ...
          IF (ES(J,K).GT.0.D0) THEN
              ...
              Z_LOW =  Z_HIGH - ES(J,K)
              !FETCH IT
              TEMP = CVSP_INTEGRATE_VOLUME_GAIA(J,1,Z_HIGH,Z_LOW,T1%R)
              !ASSIGN IT
              ASUM = 0.D0
              DO I=1,NSICLA
        ...
        DO K=1,NLAYER%I(J) ! = NOMBLAY-NUMBER OF EMPTY LAYERS
          IF (ES(J,K).LT.0.D0) THEN
            ...
            ELSE
              ...
              CALL CVSP_P_GAIA('./','ERR_ES_VSP_',  JG)
              CALL LAYERS_P_GAIA('./','ERR_ES_LAY_', JG)
              CALL PLANTE(1)
            ENDIF
          END IF
...
!         LOOP ALL FRACTIONS
     ...
     &         (AVAIL(J,K,I) .LT. 0.D0 - ZERO)) THEN
              ...
              CALL CVSP_P_GAIA('./','ERR_AVAIL_VSP_',  JG)
              CALL LAYERS_P_GAIA('./','ERR_AVAIL_LAY_', JG)
              CALL PLANTE(1)
            ENDIF
...
!       FINAL DEPTH CHECK
        IF( ABS(SUMES-ZSPACE).GT.ZERO) THEN
          ...
          CALL CVSP_P_GAIA('./','ERR_SUM_ES_VSP_', JG)
          CALL LAYERS_P_GAIA('./','ERR_SUM_ES_LAY_', JG)
          CALL PLANTE(1)
        ENDIF
      ENDDO ! J  = NODES
!
