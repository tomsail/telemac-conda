...
!>@param[in] I Index of a fraction
!>@param[in] DZFCL Value of a fraction in cm !
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
!
      USE INTERFACE_GAIA_BEDLOAD, EX => CVSP_RM_FRACTION_GAIA
...
!
      ...
      IF (CVSP_DB_GAIA(JG,0)) CALL CVSP_P_GAIA('./',OCSTR,JG)
!
...
!
          IF(EROSUM < PROV) THEN
            IF(CVSP_DB_GAIA(JG,0)) 
     &        CALL CVSP_P_GAIA('./VSP/','RFT_C2_',JG)
!
!-----------------------------------------------------------------------
...
!
            IF(CVSP_DB_GAIA(JG,0)) 
     &        CALL CVSP_P_GAIA('./VSP/','RFT_C3_',JG)
!
            IF(CVSP_CHECK_F_GAIA(J,PRO_MAX(J)-CNTR,'RMF:CAS2 B')) THEN
            ENDIF
!
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      IF (CVSP_DB_GAIA(JG,0)) CALL CVSP_P_GAIA('./VSP/','RFT_C4_',JG)
!
!-----------------------------------------------------------------------
...
!     1 POINT MORE / MEANS ONE POINT DEEPER AFFECTED
      PRO_MAX(J) = PRO_MAX(J) + 1
!
      IF(CVSP_DB_GAIA(JG,0)) CALL CVSP_P_GAIA('./VSP/','TEST4_',JG)
!
!-----------------------------------------------------------------------
...
!
      ENDDO
!
      IF(CVSP_DB_GAIA(JG,0)) CALL CVSP_P_GAIA('./VSP/','TEST5_',JG)
!
!-----------------------------------------------------------------------
...
!     ADDITIONAL CHECK FOR PRO_F
      DO K = 1, PRO_MAX(J)
        RET = CVSP_CHECK_F_GAIA(J,K,'rm_fraction: ')
      ENDDO
!
      IF(CVSP_DB_GAIA(JG,0)) CALL CVSP_P_GAIA('./','RFTLOW_',JG)
      CALL CVSP_COMPRESS_CLEAN_GAIA(J)
      IF(CVSP_DB_GAIA(JG,0)) CALL CVSP_P_GAIA('./','RFTCCL_',JG)
!
!-----------------------------------------------------------------------
!
      RETURN
