!                   ************************************
                    RECURSIVE FUNCTION CVSP_CHECK_F_GAIA
!                   ************************************
!
...
!>@param[in] K Index of a section in vertical sorting profile
...
!-----------------------------------------------------------------------
     ...
     &     SOMETEXT,JG,K,I,PRO_F(J,K,I)
          IF(PRO_F(J,K,I).GE.-1.D-3) THEN
            PRO_F(J,K,I) = 0.D0
          ELSE
            CALL CVSP_P_GAIA('./','PRO_F.lt'//SOMETEXT, J)
...
! precise as it is diveded by (1-reomoved fraction) which can be nearly zero...
          IF((1.D0-PRO_F(J,K,I)).GE.-1.D0) THEN
            PRO_F(J,K,I) = 1.D0
          ELSE
            CALL CVSP_P_GAIA('./','PRO_F.gt'//SOMETEXT, J)
...
!-----------------------------------------------------------------------
      IF(RET .EQV. .FALSE.) THEN
        RET = CVSP_CHECK_F_GAIA(J,K,'ReCheck   ')
      ENDIF
!
!-----------------------------------------------------------------------
!
