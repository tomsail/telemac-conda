...
!
          BSUM = 0.D0
          DO I=1,NSICLA
            BSUM = DCLA(I)*AVAIL(J,K,I) + BSUM
          ENDDO
!
          DO I=1,NSICLA
            WRITE (ID,'(I8,1X,I4,1X,7(G15.8,1X))')
     &      JG,(NLAYER%I(J)-K+1),DCLA(I),AT,DEPTH,
...
!
      DO I=1,NSICLA
        BSUM = DCLA(I)*AVAIL(J,NLAYER%I(J),I) + BSUM
      ENDDO
!
      DO I=1,NSICLA
            MYFRA = 0.D0
            IF (I==1) MYFRA = 1.D0
            WRITE (ID,'(I8,1X,I4,1X,7(G15.8,1X))')
     &      JG,0,DCLA(I),AT,DEPTH,MYFRA,X(J),Y(J),BSUM
      ENDDO
!
      CLOSE(ID)
      ENDIF
